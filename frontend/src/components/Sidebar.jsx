...
const ThemeToggle = () => {
  ...
  const cycle = () => {
    // Cycle : council → next-dark → light → council
    const order = ['council', 'next-dark', 'light'];
    const next = order[(order.indexOf(theme) + 1) % order.length];
    setTheme(next);
    document.documentElement.setAttribute('data-theme', next);
    localStorage.setItem('llmc-theme', next);
  };
  const label = theme === 'light' ? '☀ LIGHT' : theme === 'next-dark' ? '● DEEP' : '◐ COUNCIL';
  return <button onClick={cycle} className="theme-toggle" title="Changer le thème">{label}</button>;
};
  ...
  return (
    <div className="sidebar">
      ...
      <div className="conversation-list">
        ...
        {unassigned.map(conv => {
          const isStreaming = !!activeStreams[conv.id];
          return (
            <div key={conv.id}
              className={`conversation-item ${conv.id === currentConversationId ? 'active' : ''}${draggedConvId === conv.id ? ' dragging' : ''}${isStreaming ? ' streaming' : ''}`}
              draggable
              onDragStart={e => onDragStart(e, conv.id)}
              ...
              {isStreaming ? (
                <span className="stream-indicator">
                  <span className="status-dot active" /> en cours...
                </span>
              ) : (
                <div className="conversation-meta">{conv.message_count} messages</div>
              )}
              <div className="conv-actions">
                {isStreaming && (
                  <button className="abort-stream-btn"
                    onClick={e => { e.stopPropagation(); onAbortStream?.(conv.id); }}
                    title="Annuler la génération">✕</button>
                )}
                <button className="delete-conversation-btn"
      ...
      <div className="sidebar-footer">
        ...
        {user && (
          <div className="sidebar-user">
            <div className="sidebar-user-info" onClick={onOpenProfile} title="Mon profil" style={{ cursor: 'pointer' }}>
