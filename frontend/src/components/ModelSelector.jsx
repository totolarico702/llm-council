...
export default function ModelSelector({ selectedModels, onModelsChange, webSearchMode, onWebSearchModeChange, disabled }) {
  ...
  const [modelErrors, setModelErrors] = useState({}); // { model_id: true } — modèles indisponibles
  ...
  const fetchPipelines = () => {
    apiFetch(ROUTES.groups.list)
      ...
      .then(data => {
        ...
        // Vérifier la santé des modèles utilisés dans les pipelines
        const allModelIds = [...new Set(
          data.flatMap(p => (p.nodes || []).map(n => n.model).filter(Boolean))
        )];
        if (allModelIds.length > 0) {
          apiFetch(`${ROUTES.models.status}?models=${allModelIds.join(',')}`)
  ...
  const pipelineHasError = (pipeline) =>
    (pipeline.nodes || []).some(n => n.model && modelErrors[n.model]);
      ...
      <div className="group-pills-bar">
        {pipelines.map(g => (
          <div key={g.id} className={`group-pill${activePipelineId === g.id ? ' active' : ''}`}>
            <span onClick={() => applyPipeline(g)}>
              {g.name}
              {pipelineHasError(g) && <span className="pipeline-error-indicator" title="Un modèle de ce pipeline est indisponible">⚠</span>}
            </span>
            {isAdmin && (
              <button
                ...
                return (
                  ...
                  >
                    <div className="group-picker-name">
                      {g.name}
                      {pipelineHasError(g) && (
                        <span className="pipeline-error-indicator" title="Un modèle de ce pipeline est indisponible">⚠</span>
                      )}
