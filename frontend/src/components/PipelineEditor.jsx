...
// Injecte les nœuds I/O (PROMPT + RÉPONSE) s'ils sont absents
function withIONodes(nodes, cfg = {}) {
  const result = [...nodes];
  if (!result.some(n => n.node_type === 'prompt')) {
    result.unshift({
      id: '__prompt__', node_type: 'prompt', inputs: [],
      x: cfg.prompt_pos?.x ?? 20,
      y: cfg.prompt_pos?.y ?? 200,
    });
  }
  if (!result.some(n => n.node_type === 'response')) {
    result.push({
      id: '__response__', node_type: 'response', inputs: [],
      x: cfg.response_pos?.x ?? 1100,
      y: cfg.response_pos?.y ?? 200,
    });
  }
  return result;
}
...
function PromptResponsePanel({ type, model, availableModels, onChange, onClose }) {
  return (
    <div className="pe-np">
      <div className="pe-np-head" style={{ borderColor: '#b8941f' }}>
        <span className="pe-np-role" style={{ color: '#b8941f' }}>
          {type === 'prompt' ? '⤵ PROMPT' : '⤴ RÉPONSE'}
        </span>
        <button className="pe-np-close" onClick={onClose}>✕</button>
      </div>
      <div className="pe-np-body">
        <div className="pe-np-field">
          ...
          <select className="pe-np-select" value={model || ''}
            onChange={e => onChange(e.target.value)}>
        ...
        <div className="pe-np-field">
          <p className="pe-np-hint">
            {type === 'prompt'
              ? 'Modèle utilisé pour analyser et reformuler le prompt initial avant injection dans le pipeline.'
              : 'Modèle utilisé pour formater et synthétiser la réponse finale du pipeline.'}
          </p>
        </div>
      </div>
    </div>
...
export default function PipelineEditor({ group, onSave, onClose }) {
  ...
  const [modelErrors, setModelErrors]   = useState({}); // { model_id: 'unavailable' }
  ...
  // Panneaux Prompt / Réponse fixes
  const [promptModel,   setPromptModel]   = useState('');
  const [responseModel, setResponseModel] = useState('');
  const [selectedPanel, setSelectedPanel] = useState(null); // 'prompt' | 'response' | null
  ...
  // Vérifier la disponibilité des modèles utilisés dans les nœuds
  useEffect(() => {
    const isLocal = (m) => m.startsWith('ollama/') || m.startsWith('local/');
    // Cloud seulement — les modèles locaux sont vérifiés par le backend Ollama
    const cloudIds = [...new Set(
      nodes.map(n => n.model).filter(m => m && !isLocal(m))
    )];
    if (cloudIds.length === 0) { setModelErrors({}); return; }
    apiFetch(`${ROUTES.models.status}?models=${cloudIds.join(',')}`)
      .then(r => r && r.ok ? r.json() : {})
      .then(status => {
        const errors = {};
        for (const [id, ok] of Object.entries(status)) {
          if (!ok) errors[id] = 'unavailable';
        }
  ...
  useEffect(() => {
    ...
    const cfg        = group?.cog?.config || {};
    const fallbackModel = group?.models?.[0] || 'google/gemini-2.0-flash-001';

    // Extraire modèles Prompt/Réponse depuis config ou nœuds hook/chairman legacy
    const hookNode     = loadNodes.find(n => n.id === 'hook'     || n._source);
    const chairmanNode = loadNodes.find(n => n.id === 'chairman' || n.role === 'chairman');
    setPromptModel(cfg.prompt_model     || hookNode?.model     || fallbackModel);
    setResponseModel(cfg.response_model || chairmanNode?.model || fallbackModel);

    if (loadNodes.length > 0) {
      const ns = withIONodes(nodesFromBackend(loadNodes), cfg);
      setNodes(ns);
      ...
      // Pipeline vide — un nœud explorateur au centre + nœuds I/O
      setNodes(withIONodes([{
  ...
  const onNodeMouseDown = useCallback((e, nodeId) => {
    ...
    // Nœuds I/O → ouvrir directement le panneau de config correspondant
    if (node.node_type === 'prompt')   setSelectedPanel('prompt');
    else if (node.node_type === 'response') setSelectedPanel('response');
  ...
  const onCanvasWheel = useCallback((e) => {
    ...
    setZoom(prev => {
      const next = Math.min(3, Math.max(0.05, prev * delta));
  ...
  const loadFromPipeline = (pipeline) => {
    ...
    const cfg        = pipeline.cog?.config || {};
    setPromptModel(cfg.prompt_model || '');
    setResponseModel(cfg.response_model || '');
    if (loadNodes.length > 0) {
      setNodes(withIONodes(nodesFromBackend(loadNodes), cfg));
  ...
  const addNode = (type = 'llm') => {
    if (type === 'tool')  { addToolNode();  return; }
    if (type === 'agent') { addAgentNode(); return; }
    const existingIds = nodes.map(n => n.id);
    const newNode = {
  ...
  const deleteSelected = () => {
    ...
    // Les nœuds I/O sont protégés
    if (selectedId === '__prompt__' || selectedId === '__response__') return;
  ...
  const handleSave = async () => {
    ...
    try {
      // Exclure les nœuds I/O du payload backend
    const backendNodes = nodes
      .filter(n => n.node_type !== 'prompt' && n.node_type !== 'response')
      .map(({ _source, ...n }) => {
        const base = {
          id:        n.id,
    ...
    const isNew = !pipelineId;
      const payload = {
        name,
        cog: { cog_version: '1.0', nodes: backendNodes, edges: backendEdges,
               config: {
                 prompt_model:   promptModel,
                 response_model: responseModel,
                 prompt_pos:   promptNode   ? { x: Math.round(promptNode.x),   y: Math.round(promptNode.y)   } : undefined,
                 response_pos: responseNode ? { x: Math.round(responseNode.x), y: Math.round(responseNode.y) } : undefined,
               } },
      };
  ...
  const sanitizeNodes = (nodes) => nodes.map(n => ({
    ...n,
    type:  n.type  || n.data?.nodeType || 'llm',
    model: n.model || n.data?.model    || 'mistralai/mistral-medium-3',
  ...
  const handleExportCog = async () => {
    ...
    if (cog.nodes) cog.nodes = sanitizeNodes(cog.nodes)
  ...
  const handleImportFile = (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (parsed.nodes) parsed.nodes = sanitizeNodes(parsed.nodes)
  ...
  const handlePasteJSON = async () => {
    try {
      ...
      await previewImport(parsed);
      setImportModal(true);
    } catch { showToast('Presse-papier vide ou JSON invalide', 'err'); }
  };

  // ── Dupliquer le pipeline ──────────────────────────────────────────────────
  const handleDuplicate = async () => {
    if (!pipelineId) { showToast('Sauvegardez d\'abord le pipeline', 'err'); return; }
    const newName = window.prompt('Nom du duplicate :', `${pipelineName} — copie`);
    if (!newName) return;
    const res = await apiFetch(ROUTES.pipelines.create, {
      method: 'POST',
      body: JSON.stringify({
        name: newName,
        cog: { cog_version: '1.0', nodes: nodes.map(({ _source, ...n }) => n), edges,
               config: { prompt_model: promptModel, response_model: responseModel } },
      }),
    });
    if (res?.ok) { showToast(`Pipeline "${newName}" créé`, 'ok'); onSave?.(); }
    else showToast('Erreur duplication', 'err');
  };

  // ── Plein écran ────────────────────────────────────────────────────────────
  const toggleFullscreen = () => {
    const el = document.querySelector('.pe-modal');
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.ctrlKey && !e.shiftKey && (e.key === '+' || e.key === '=')) { e.preventDefault(); addNode('llm'); }
      if (e.ctrlKey && e.shiftKey  && (e.key === '+' || e.key === '=')) { e.preventDefault(); addNode('tool'); }
      if (e.key === 'Delete' && selectedId) { e.preventDefault(); deleteSelected(); }
      if (e.ctrlKey && e.key === 'i') { e.preventDefault(); setImportModal(true); }
      if (e.ctrlKey && e.key === 'e') { e.preventDefault(); handleExportCog(); }
      if (e.ctrlKey && e.key === 'c' && !selectedId) { e.preventDefault(); handleCopyJson(); }
      if (e.ctrlKey && e.key === 'v' && !selectedId) { e.preventDefault(); handlePasteJSON(); }
      if (e.ctrlKey && e.key === 'l') { e.preventDefault(); applyAllLocal(); }
      if (e.ctrlKey && e.key === 'd') { e.preventDefault(); handleDuplicate(); }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [selectedId, nodes, edges, promptModel, responseModel]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="pe-overlay" onClick={e => e.target === e.currentTarget && onClose?.()}>
      <div className="pe-modal">
        ...
        <div className="pe-toolbar">
          ...
          <button className="pe-toolbar-btn pe-btn-agent" onClick={addAgentNode}
            title="Ajouter un nœud agent inter-agents V3">🤖 Agent</button>
          <div className="pe-toolbar-sep" />
          ...
          <button className="pe-toolbar-btn" onClick={toggleFullscreen} title="Basculer plein écran">⛶ Plein écran</button>
          <button className="pe-toolbar-btn" onClick={() => setShowShortcuts(true)} title="Raccourcis clavier">⌨ Raccourcis</button>
          ...
          >
            ...
            {nodes.filter(n => n.node_type !== 'prompt' && n.node_type !== 'response').map(node => {
              ...
              return (
                <div
                  key={node.id}
                  className={`pe-node${isSelected ? ' pe-node-selected' : ''}${isTerminal ? ' pe-node-terminal' : ''}${node._source ? ' pe-node-source' : ''}${shiftTarget === node.id ? ' pe-node-shift-target' : ''}${node.node_type === 'tool' ? ' pe-node-tool' : ''}${(node.node_type === 'prompt' || node.node_type === 'response') ? ' pe-node-io' : ''}`}
                  style={{
                    left: node.x, top: node.y,
                    width: NODE_W, height: NODE_H,
                    '--nc': (node.node_type === 'prompt' || node.node_type === 'response')
                      ? '#b8941f'
                      : node.node_type === 'tool'
                ...
                >
                  {/* Port entrée — masqué pour nœud source et nœud prompt */}
                  {!node._source && node.node_type !== 'prompt' && (
                  ...
                  <div className="pe-node-content">
                    {node.node_type === 'prompt' ? (
                      ...
                      <>
                        <div className="pe-node-role" style={{ color: '#b8941f' }}>🤖 Agent</div>
                        <div className="pe-node-id">{node.id}</div>
                        <div className="pe-node-model">{node.agent_id ? node.agent_id.slice(0,18) + '…' : '(non configuré)'}</div>
                      </>
                    ) : node.node_type === 'tool' ? (() => {
                      const ti = getToolInfo(node.tool_type);
                      return (<>
                  ...
                  {node.model && modelErrors[node.model] && (
                    <div className="node-error-badge" title="Modèle indisponible sur OpenRouter">
                      ⚠ Modèle indisponible
                    </div>
                  )}

                  {/* Port sortie — masqué pour nœud response */}
                  {node.node_type !== 'response' && (
        ...
        {/* ── Colonne droite : Config nœud ou Prompt/Réponse (slide-in) ── */}
        <div className={`pe-side${(selectedNode || selectedPanel) ? '' : ' pe-side-hidden'}`} onClick={e => e.stopPropagation()}>
          {selectedPanel && (
            <PromptResponsePanel
              type={selectedPanel}
              model={selectedPanel === 'prompt' ? promptModel : responseModel}
              availableModels={availableModels}
              onChange={val => {
                if (selectedPanel === 'prompt') { setPromptModel(val); setDirty(true); }
                else { setResponseModel(val); setDirty(true); }
              }}
        ...
        {showShortcuts && (
          <div className="pe-shortcuts-overlay" onClick={() => setShowShortcuts(false)}>
            <div className="pe-shortcuts-modal" onClick={e => e.stopPropagation()}>
              <div className="pe-shortcuts-header">
                <span>⌨ Raccourcis clavier</span>
                <button onClick={() => setShowShortcuts(false)}>✕</button>
              </div>
              <table className="pe-shortcuts-table">
                <tbody>
                  {[
                    ['Ctrl+S',     'Sauvegarder'],
                    ['Ctrl++',     'Nouveau nœud LLM'],
                    ['Ctrl+Maj++', 'Nouvel outil'],
                    ['Suppr',      'Supprimer nœud sélectionné'],
                    ['Ctrl+I',     'Importer .cog'],
                    ['Ctrl+E',     'Exporter .cog'],
                    ['Ctrl+C',     'Copier JSON'],
                    ['Ctrl+V',     'Coller JSON'],
                    ['Ctrl+L',     'Tout en local'],
                    ['Ctrl+D',     'Dupliquer pipeline'],
                    ['⛶',          'Plein écran'],
                    ['Molette',    'Zoom avant / arrière'],
                    ['Alt+Drag',   'Déplacer la vue (pan)'],
                  ].map(([key, desc]) => (
                    <tr key={key}>
                      <td className="pe-shortcut-key">{key}</td>
                      <td className="pe-shortcut-desc">{desc}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
