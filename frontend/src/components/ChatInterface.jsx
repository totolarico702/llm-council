...
function ExecutionPanel({ dag, open, onClose, userQuery, onReplay, conversationId }) {
  const [pipelineScore, setPipelineScore] = useState(null);
  ...
  const pipelineMeta = dag.pipeline_meta || {};
  const isDone = !!dag.execution_order?.length;

  const handleStarScore = async (star) => {
    if (!pipelineMeta.id) return;
    setPipelineScore(star);
    try {
      await api.submitPipelineScore(pipelineMeta.id, {
        score: star,
        conversation_id: conversationId,
      });
      setScoreSaved(true);
    } catch (e) {
      console.error('Pipeline score error:', e);
    }
  };
  ...
  return (
    <div className={`exec-panel${open ? ' open' : ''}`}>
      ...
      {/* En-tête pipeline */}
      {(pipelineMeta.name || pipelineMeta.id) && (
        <div className="exec-pipeline-header">
          <span className="exec-pipeline-label">Pipeline</span>
          <span className="exec-pipeline-name">{pipelineMeta.name || pipelineMeta.id}</span>
          {isDone && userQuery && onReplay && (
            <button
              className="exec-replay-btn"
              onClick={() => onReplay(userQuery, pipelineMeta.nodes)}
              title="Relancer avec la même requête et le même pipeline"
            >
              ↺ Relancer
            </button>
          )}
      ...
      {/* Scoring pipeline */}
      {isDone && pipelineMeta.id && (
        <div className="exec-scoring">
          <span className="exec-scoring-label">Noter ce pipeline :</span>
          <div className="exec-stars">
            {[1,2,3,4,5].map(star => (
              <button
                key={star}
                className={`exec-star-btn${pipelineScore >= star ? ' active' : ''}`}
                onClick={() => handleStarScore(star)}
                title={`${star} étoile${star > 1 ? 's' : ''}`}
              >⭐</button>
            ))}
          </div>
          {scoreSaved && <span className="exec-score-saved">✓ Noté</span>}
        </div>
      )}
    </div>
  );
...
export default function ChatInterface({
  ...
  const [pipelineInfo,   setPipelineInfo]   = useState(null); // { id, name } du pipeline sélectionné
  ...
  const handleSubmit = async (e) => {
    ...
    // Trim trailing punctuation from filenames to handle manual typing (e.g. "@rapport.pdf,")
    const mentionMatches = [...userText.matchAll(/(?:^|\s)@(\S+)/g)];
    const mentions = mentionMatches.map(m => m[1].replace(/[.,;:!?)\]]+$/, ''));
    ...
    // documentContent = pièces jointes + @mentions résolus (injecté côté backend seulement)
    ...
    // userText est le contenu affiché et stocké ; documentContent est injecté séparément dans le prompt
    const attachmentNames = attachments.map(a => a.name);

    onSendMessage(userText, selectedModels, webSearchMode, documentContent, attachmentNames, pipelineNodes, caffeineMode, pipelineInfo);
    setInput('');
    setAttachments([]);
  };
        ...
        <ModelSelector
          ...
          onModelsChange={(val) => {
            if (val && val.__pipeline_id) {
              ...
              setPipelineInfo({ id: val.__pipeline_id, name: val.name || null });
              setSelectedModels([]);
            } else {
  ...
  return (
    <div className="chat-interface">
      ...
      <ExecutionPanel
        ...
        userQuery={execMsgIdx !== null ? conversation.messages[execMsgIdx - 1]?.content : null}
        conversationId={conversation?.id}
        onReplay={(query, nodes) => {
          if (!query || !nodes) return;
          const replayInfo = conversation.messages[execMsgIdx]?.dag?.pipeline_meta;
          onSendMessage(query, [], 'none', null, [], nodes, false, replayInfo ? { id: replayInfo.id, name: replayInfo.name } : null);
        }}
      />
