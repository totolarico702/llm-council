...
from .db import (
    ...
    archive_user, reactivate_user, get_users_by_service,
...
from . import agent_manager as _agent_mgr
from . import model_advisor as _model_advisor
...
async def lifespan(app):
    ...
    await _check_ollama()
    # Démarrer le scheduler d'agents V3
    try:
        _agent_mgr.start_scheduler()
    except Exception as _e:
        print(f"[WARN] Scheduler agents non démarré : {_e}")
...
class SendMessageRequest(BaseModel):
    ...
    pipeline_id: Optional[str] = None    # ID du pipeline utilisé
    pipeline_name: Optional[str] = None  # Nom du pipeline (pour affichage)
...
async def route_create_user(body: CreateUserRequest, _: dict = Depends(require_admin)):
    return create_user(
        body.login, body.password, body.role, body.service_id,
        first_name=body.first_name, last_name=body.last_name,
        email=body.email, departments=body.departments,
    )
...
@api_v1.post("/admin/users/{user_id}/soft-archive", status_code=200)
async def route_soft_archive_user(user_id: str, admin: dict = Depends(require_admin)):
    user = db.get_user_by_id(user_id)
    if not user:
        raise HTTPException(status_code=404, detail="Utilisateur introuvable")
    if user.get("role") == "admin":
        raise HTTPException(status_code=400, detail="Impossible d'archiver un admin")
    return archive_user(user_id, admin["login"])

@api_v1.post("/admin/users/{user_id}/reactivate", status_code=200)
async def route_reactivate_user(user_id: str, _: dict = Depends(require_admin)):
    user = db.get_user_by_id(user_id)
    if not user:
        raise HTTPException(status_code=404, detail="Utilisateur introuvable")
...
async def route_list_services(_: dict = Depends(require_admin)):
    ...
    for svc in services:
        users = get_users_by_service(svc["id"])
        active = [u for u in users if not u.get("is_archived")]
        svc["users"] = [
            {"id": u["id"], "login": u["login"],
             "first_name": u.get("first_name", ""), "last_name": u.get("last_name", "")}
            for u in active
        ]
        svc["users_count"] = len(active)
    return services
...
class PipelineScoreRequest(BaseModel):
    score: int                          # 1 à 5 étoiles
    conversation_id: Optional[str] = None
    message_id: Optional[str] = None

@api_v1.post("/pipelines/{pipeline_id}/score", status_code=201)
async def route_score_pipeline(pipeline_id: str, body: PipelineScoreRequest,
                                user: dict = Depends(get_current_user)):
    """Note un pipeline (1-5 étoiles). Score stocké en TinyDB via scorer.py."""
    if not (1 <= body.score <= 5):
        raise HTTPException(status_code=422, detail="Le score doit être entre 1 et 5")
    from .scorer import save_score
    # Normalise 1-5 → 2-10 pour cohérence avec les scores modèles
    score_norm = body.score * 2
    entry = save_score(
        ...
        user_id=user["id"],
        source="user",
        scores={"overall": score_norm, "relevance": 0, "accuracy": 0, "format": 0},
...
class AgentCreateRequest(BaseModel):
    ...
    output:      dict = {"type": "conversation", "notify_users": []}
...
@api_v1.get("/agents")
async def route_list_agents(user: dict = Depends(get_current_user)):
    agents = _agent_mgr.list_agents()
    # Les users normaux ne voient que leurs agents
    if user.get("role") != "admin":
        agents = [a for a in agents if a.get("created_by") == user["id"]]
    return agents

@api_v1.post("/agents", status_code=201)
async def route_create_agent(body: AgentCreateRequest, user: dict = Depends(get_current_user)):
    return _agent_mgr.create_agent(body.model_dump(), created_by=user["id"])

@api_v1.get("/agents/{agent_id}")
async def route_get_agent(agent_id: str, user: dict = Depends(get_current_user)):
    agent = _agent_mgr.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    if user.get("role") != "admin" and agent.get("created_by") != user["id"]:
        raise HTTPException(status_code=403, detail="Accès refusé")
    return agent

@api_v1.patch("/agents/{agent_id}")
async def route_update_agent(agent_id: str, body: AgentUpdateRequest,
                              user: dict = Depends(get_current_user)):
    agent = _agent_mgr.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    if user.get("role") != "admin" and agent.get("created_by") != user["id"]:
        raise HTTPException(status_code=403, detail="Accès refusé")
    updated = _agent_mgr.update_agent(agent_id, body.model_dump(exclude_none=True))
    # Réenregistrer le cron si le trigger a changé
    try:
        _agent_mgr._reload_scheduled_agents()
    except Exception:
        pass
    return updated

@api_v1.delete("/agents/{agent_id}", status_code=204)
async def route_delete_agent(agent_id: str, user: dict = Depends(get_current_user)):
    agent = _agent_mgr.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    if user.get("role") != "admin" and agent.get("created_by") != user["id"]:
        raise HTTPException(status_code=403, detail="Accès refusé")
    _agent_mgr.delete_agent(agent_id)

@api_v1.post("/agents/{agent_id}/trigger")
async def route_trigger_agent(agent_id: str, body: dict = {},
                               user: dict = Depends(get_current_user)):
    agent = _agent_mgr.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    context = {**(body or {}), "triggered_by": user["id"]}
    try:
        exec_id = await _agent_mgr.trigger_agent(agent_id, context, "manual")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return {"execution_id": exec_id, "status": "started"}

@api_v1.post("/agents/{agent_id}/pause")
async def route_pause_agent(agent_id: str, _: dict = Depends(require_admin)):
    agent = _agent_mgr.pause_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    return agent

@api_v1.post("/agents/{agent_id}/resume")
async def route_resume_agent(agent_id: str, _: dict = Depends(require_admin)):
    agent = _agent_mgr.resume_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    try:
        _agent_mgr._register_cron_job(agent)
    except Exception:
        pass
    return agent

@api_v1.get("/agents/{agent_id}/executions")
async def route_agent_executions(agent_id: str, limit: int = 20,
                                  user: dict = Depends(get_current_user)):
    agent = _agent_mgr.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent introuvable")
    return _agent_mgr.list_executions(agent_id, limit=limit)

# ── Catalogue agents métier V3 ────────────────────────────────────────────────

@api_v1.get("/catalog/agents")
async def route_catalog_agents(domain: Optional[str] = None,
                                _: dict = Depends(get_current_user)):
    """Retourne les templates d'agents du catalogue métier."""
    import json as _json
    from pathlib import Path as _Path
    catalog_dir = _Path(__file__).parent / "agent_catalog"
    templates = []
    for cog_file in sorted(catalog_dir.rglob("*.cog.json")):
        try:
            data = _json.loads(cog_file.read_text(encoding="utf-8"))
            cat  = data.get("catalog", {})
            if domain and cat.get("domain") != domain:
                continue
            templates.append({
                "id":           cog_file.stem.replace(".cog", ""),
                "path":         str(cog_file.relative_to(catalog_dir)),
                **cat,
...
@api_v1.get("/catalog/agents/domains")
async def route_catalog_domains(_: dict = Depends(get_current_user)):
    """Retourne la liste des domaines disponibles dans le catalogue."""
    from pathlib import Path as _Path
    catalog_dir = _Path(__file__).parent / "agent_catalog"
    domains = sorted({d.name for d in catalog_dir.iterdir() if d.is_dir()})
    return domains

# ── Model Advisor V3 ──────────────────────────────────────────────────────────

@api_v1.post("/advisor/recommend-node")
async def route_recommend_node(body: dict, user: dict = Depends(get_current_user)):
    """Recommande la configuration optimale d'un nœud pour une tâche."""
    description = (body or {}).get("description", "")
    if not description:
        raise HTTPException(status_code=400, detail="description requise")
    rec = _model_advisor.recommend_node(description, user_context={"user_id": user["id"]})
    return rec

@api_v1.get("/advisor/model-capabilities")
async def route_model_capabilities(_: dict = Depends(get_current_user)):
    """Retourne la base de connaissance des capacités des modèles."""
    return _model_advisor.MODEL_CAPABILITIES

@api_v1.get("/advisor/task-types")
async def route_task_types(_: dict = Depends(get_current_user)):
    """Retourne les types de tâches supportés et leurs mappings modèles."""
    return {
        "tasks": list(_model_advisor.TASK_TO_MODEL_MAPPING.keys()),
        "mappings": _model_advisor.TASK_TO_MODEL_MAPPING,
        "rag_recommended": _model_advisor.RAG_RECOMMENDED_FOR,
        "web_search_recommended": _model_advisor.WEB_SEARCH_RECOMMENDED_FOR,
    }
...
async def health_models(models: str = ""):
    ...
    - Modèles cloud  (pas de préfixe ollama/ ou local/) → OpenRouter API
    - Modèles locaux (ollama/ ou local/)                → Ollama /api/tags
    Retourne { model_id: true (ok) | false (indisponible) }
    """
    from .config import OPENROUTER_API_KEY
    import httpx
    ...
    local_models = [m for m in model_list
                    if m.startswith("ollama/") or m.startswith("local/")]
    cloud_models  = [m for m in model_list if m not in local_models]
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if local_models:
            try:
                ollama_url = os.getenv("OLLAMA_URL", "http://localhost:11434")
                r_ollama = await client.get(f"{ollama_url}/api/tags", timeout=3.0)
                if r_ollama.status_code == 200:
                    known = {m["name"] for m in r_ollama.json().get("models", [])}
                    for m in local_models:
                        # "ollama/mistral:latest" → "mistral:latest"
                        name = m.split("/", 1)[-1]
                        result[m] = any(k.startswith(name.split(":")[0]) for k in known)
                else:
                    for m in local_models:
                        result[m] = False
            except Exception:
                # Ollama non disponible — on indique False (modèles non testables)
                for m in local_models:
                    result[m] = False
        ...
        if cloud_models:
            try:
                r_or = await client.get(
                    "https://openrouter.ai/api/v1/models",
                    headers={"Authorization": f"Bearer {OPENROUTER_API_KEY}"},
                )
                if r_or.status_code == 200:
                    available = {m["id"] for m in r_or.json().get("data", [])}
                    for m in cloud_models:
                        result[m] = m in available
                else:
                    # OpenRouter injoignable — ne pas marquer comme erreur
                    for m in cloud_models:
                        result[m] = True
            except Exception:
                for m in cloud_models:
...
async def send_message(conversation_id: str, request: SendMessageRequest,
    ...
    if request.pipeline_nodes:
        ...
        council_content = request.content
        ...
        stage1_results = await stage1_collect_responses(
            council_content, council_models, request.web_search_mode, history)
        stage2_results, label_to_model = await stage2_collect_rankings(
            council_content, stage1_results, council_models, history)
        aggregate_rankings = calculate_aggregate_rankings(stage2_results, label_to_model)
        stage3_result = await stage3_synthesize_final(
            council_content, stage1_results, stage2_results,
...
async def send_message_stream(conversation_id: str, request: SendMessageRequest,
    ...
    async def event_generator():
        try:
            ...
            # Pour le mode council classique, le contexte documentaire est prépendé
            # à la query car gather_opinions/peer_review/synthesize ne prennent pas document_content.
            # Le mode DAG gère document_content directement dans compose_node_prompt.
            council_content = user_content
            if request.document_content:
                council_content = (
                    f"[DOCUMENT PROVIDED BY USER]\n{request.document_content}\n[END DOCUMENT]\n\n"
                    f"{user_content}"
...
# ── V3 : router public (deliberate + API keys) ────────────────────────────────
from .deliberate_router import router as _deliberate_router
app.include_router(_deliberate_router)

# ── V3 : servir le frontend buildé depuis /app/frontend/dist (Docker) ─────────
...
if _DIST.is_dir():
    ...
    async def _spa_fallback(full_path: str):
        """Retourne index.html pour toutes les routes SPA non-API."""
